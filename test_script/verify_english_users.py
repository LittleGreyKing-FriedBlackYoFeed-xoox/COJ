...
def verify_english_information():
    # Get all users
    ...
    for user in users:
        ...
        if contains_chinese(user.username):
            ...
            print(f"User {user.id} has Chinese characters in username: {user.username}")
        
        if contains_chinese(user.real_name):
            has_chinese = True
            chinese_fields['real_name'] += 1
            print(f"User {user.id} has Chinese characters in real_name: {user.real_name}")
        
        if contains_chinese(user.remark):
            has_chinese = True
            chinese_fields['remark'] += 1
            print(f"User {user.id} has Chinese characters in remark: {user.remark}")
...
if __name__ == "__main__":
    print("Verifying all users have English information...")
    english_only, chinese_found, role_counts = verify_english_information()
