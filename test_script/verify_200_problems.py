...
def verify_new_problems():
    ...
    try:
        ...
        # Get the 200 most recent problems
        recent_problems = problems[:200]
        ...
        print(f"\nDifficulty distribution of 200 newest problems:")
        print(f"Easy: {easy_count} ({easy_count/200*100:.1f}%)")
        print(f"Medium: {medium_count} ({medium_count/200*100:.1f}%)")
        print(f"Hard: {hard_count} ({hard_count/200*100:.1f}%)")
        ...
        print(f"\nKnowledge point distribution of 200 newest problems:")
        for kp, count in sorted(knowledge_points.items(), key=lambda x: x[1], reverse=True)[:20]:
            print(f"{kp}: {count} ({count/200*100:.1f}%)")
        
        print(f"\nTotal unique knowledge points: {len(knowledge_points)}")
        ...
        if non_english_count == 0:
            print("\nAll 200 newest problems are in English.")
        else:
